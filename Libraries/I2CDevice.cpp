...
int I2CDevice::writeBit(unsigned int registerAddress,unsigned char bitNum, unsigned char data){
	unsigned char b = readRegister(registerAddress);
	b = (data != 0) ? (b | (1 << bitNum)) : (b & ~(1 << bitNum));
	return writeRegister(registerAddress, b);
}

bool I2CDevice::writeBits(unsigned int registerAddress, unsigned char bitStart, unsigned char length, unsigned char data){
	unsigned char b;
	b = readRegister(registerAddress);
	if(b != 0){
...
int I2CDevice::readBit(unsigned int registerAddress, unsigned char bitNum){
	return (readRegister(registerAddress) & (1 << bitNum));
}


unsigned char I2CDevice::readBits(unsigned char registerAddress, unsigned char bitStart, unsigned char length) {
    // 01101001 read byte
    ...
    b = this->readRegister(registerAddress);
    if ((count = b) != 0) {
        unsigned char mask = ((1 << length) - 1) << (bitStart - length + 1);
        b &= mask;
        b >>= (bitStart - length + 1);
